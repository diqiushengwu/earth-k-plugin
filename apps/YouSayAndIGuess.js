...
export class YouSayAndIGuess extends plugin {
    ...
    async jryx(e) {
        ...
        if (fq == 0 & e.msg == '#发起你说我猜') {
            zhuren = e.user_id
            fq = 1
            e.reply('你说我猜已发起，快来加入吧')
            
        } else if (fq == 1 & e.msg != '#加入你说我猜') {
            e.reply('已经发起过了，哼哼啊啊啊啊啊')
            return
        }
        else if (fq == 0 & e.msg == '#加入你说我猜') {
            e.reply('游戏还没发起呢')
            return
        }
    ...
    async ksyx(e) {
        ...
        if (e.msg.includes('猜测') & game == 1) {
            ...
            if (caice == daan) {
               ...
               e.reply(["恭喜", segment.at(e.user_id), '回答正确，加1分']);
				...
				console.log(huihe)
				console.log(wjname[huihe])
                e.reply(['正确答案是：' + daan + '\n现在进入下一轮', '\n当前发言:',segment.at(wanjia[huihe]),'请私信机器人发送 发词,或者发送 写答案+你出的词'])
        ...
        if (game == 0 & e.msg == '#开始你说我猜' & fq == 1) {
            ...
            e.reply([msg2, '\n当前发言:', segment.at(wanjia[0]),'请私信机器人发送 发词,或者发送 写答案+你出的词'])
